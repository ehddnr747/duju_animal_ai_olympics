...
env = UnityEnvironment(worker_id=int(parser.parse_args().env),
        base_port=5005 + int(parser.parse_args().env)+ 20,
...
exp_title = "Animal_AI_World_Proposed3_"+ env_file.split(".")[0]
print(exp_title)
...
model_data_collection_episode = 333
...
for train_step in range(1,100000+1):
...
for epi_i in range(1, max_episode + 1):
    ...
    epsilon = max(1.0 * (1 - epi_i/100),0.05*(1 - epi_i/max_episode))
