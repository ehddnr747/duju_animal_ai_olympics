...
class ReplayBuffer(object):
    def __init__(self, state_dim, action_dim, buffer_size):  # Image state will be handled at ImageBuffer
        ...
        self.a_buffer = np.zeros([buffer_size, action_dim], dtype = np.int)
        self.r_buffer = np.zeros([buffer_size, 1])
        self.t_buffer = np.zeros([buffer_size, 1], dtype = np.bool)
    ...
    def sample_sequence_uniform(self, batch_size, image_buffer):
        batch_size = min(self.count, batch_size)

        sample_start_idx = np.random.choice(self.count, batch_size, replace=False)

        sample_idx = []

        for start_idx in sample_start_idx:
            sample_idx.extend(np.arange(start_idx, start_idx + 40) % self.count)

        a_return = self.a_buffer[sample_idx].reshape(40, batch_size, self.action_dim)
        r_return = self.r_buffer[sample_idx].reshape(40, batch_size, 1)
        t_return = self.t_buffer[sample_idx].reshape(40, batch_size, 1)

        s_return = self.s_buffer[sample_idx]

        s_frames = []
        s2_frames = []

        for s_idx in s_return:
            s_frame = image_buffer.get_current_image(int(s_idx)) #[3, 84, 84]
            s2_frame = image_buffer.get_current_image(int(s_idx)+1)

            s_frames.append(s_frame)
            s2_frames.append(s2_frame)

        s_return = np.array(s_frames).reshape([40, batch_size, 3, 84, 84])
        s2_return = np.array(s2_frames).reshape([40, batch_size, 3, 84, 84])


        return s_return, a_return, r_return, t_return, s2_return, batch_size
