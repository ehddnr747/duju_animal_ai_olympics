...
class DiscreteSAC(nn.Module):
    def __init__(self, state_dim, action_dim, lr, device):
        ...
        self.fc2 = nn.Linear(256, action_dim).to(device)
        # self.fc3 = nn.Linear(256, action_dim).to(device)

        # Q2
        self.fc4 = nn.Linear(state_dim, 256).to(device)
        self.fc5 = nn.Linear(256, action_dim).to(device)
...
def train_discrete_SAC_max(Q_main, Q_target, replay_buffer, batch_size, gamma, alpha):
    device = Q_main.device
    state_dim = Q_main.state_dim
    action_dim = Q_main.action_dim
    ...
    with torch.no_grad():
        ...
        main_entropy_s2 = alpha * main_policy_distribution_s2.entropy().view(-1, 1)  # H[s']
        ...
        y_v = main_entropy_s2 + q_target_min
        y_q = r_batch + gamma * y_v
